        ...
        <section className = {`h-full w-full gap-20 max-md:gap-5 flex flex-col items-center pt-12 pb-36 px-28 max-md:py-6 max-md:px-4 ${isDark ? 'bg-black' : 'bg-white'}`}>
            <div className = "flex flex-row  ">
                <motion.h1
                    ...
                    <Link
                        ...
                        spy = {true}
                        activeClass = "active"
                        className = {`font-['Work Sans'] font-semibold text-xl max-md:text-sm leading-[140%] tracking-[-1%] flex flex-row gap-2 items-center ${isDark?`text-white`:`text-black`}`}>
            ...
            <div className={`flex items-center justify-center w-full `}>
                ...
                <motion.p
                    ...
                    className = {`font-[Work Sans] font-medium text-xl max-md:text-sm leading-[140%] max-md:leading-6 tracking-[-1%]  text-center max-md:w-52 max-md:hidden ${isDark?`text-white`:`text-dark`}`}>Hello, I'm Joey, an online product designer focusing on brand identity, advertising, and no-code instruments.</motion.p>
                <motion.p
                    ref = {ref}
                    initial={{opacity: 0}}
                    animate={{opacity: 1}}
                    transition={{duration:0.8, delay:0.5}}
                    className = {`font-[Work Sans] font-medium text-xl max-md:text-sm leading-[140%] max-md:leading-6 tracking-[-1%] max-w-117.75 max-md:w-52 md:hidden text-center ${isDark?`text-white`:`text-dark`}`}>Hey there, I'm Joey, a digital product designer specializing in branding, marketing, and no-code tools.</motion.p>
