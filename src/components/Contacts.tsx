...
export default function Contacts(){
    ...
    return(
        <section className={`h-full px-28 max-md:px-4 py-12 flex items-center justify-center max-md:border-t ${isDark?`bg-black`:`bg-white`}`}>
            <div className={`flex flex-col gap-37.5 max-md:gap-12 items-center w-full`}>

                <div className={`gap-12 max-md:gap-6 flex flex-col items-center justify-center`}>
                    <span className={`font-['Work Sans'] font-semibold text-[140px] max-md:text-5xl leading-[110.00000000000001%] max-md:leading-[120%] tracking-[-2%] max-md:tracking-[-2%] ${isDark?`text-white`:`text-black`}`}>Let’s talk!</span>
                    <button className = {`rounded-[144px] flex flex-row items-center justify-center gap-2 max-w-65 py-8 max-md:py-6 px-13.25 ${isDark?`bg-[#141414]`:`bg-black`}`}>
                ...
                <div className="flex flex-row max-md:flex-col-reverse gap-190 max-md:gap-4">

                    <p className={`font-['Work Sans'] font-medium text-sm max-md:text-[10px] leading-6 tracking-[-1%] max-md:text-center ${isDark?`text-white`:`text-black`}`}>2023 © — Made by Djectstudio</p>
